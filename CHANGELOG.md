...
- Enh: Custom side menu via `toir427\admin\Module::items` (mdmunir).
...
- Chg: Using `VarDumper::export` to save `data` of `toir427\models\AuthItem` (mdmunir).
...
- Chg: Can customize return of `toir427\admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `toir427\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `toir427\admin\Module::allowActions`. Set access control directly with `toir427\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `toir427\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
